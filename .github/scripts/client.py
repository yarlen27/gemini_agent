...
while True:
    ...
    try:
        ...
        if action == "finish":
            ...
            # Commit changes and prepare PR URL
            try:
                ...
                repo_owner = os.getenv("GITHUB_REPOSITORY").split("/")[0] # e.g., yarlen27
                repo_name = os.getenv("GITHUB_REPOSITORY").split("/")[1] # e.g., gemini_agent
                
                pr_url_base = f"https://github.com/{repo_owner}/{repo_name}/compare/{BRANCH_NAME}?expand=1&title="
                pr_title_encoded = requests.utils.quote(f"feat(issue-{ISSUE_NUMBER}): {ISSUE_TITLE}")
                pr_body_encoded = requests.utils.quote(f"Resolves #{ISSUE_NUMBER}\n\nThis PR was automatically generated by Gemini.\n\nAgent's final message:\n{agent_response.get('message', 'No message')}")

                pr_creation_url = f"{pr_url_base}{pr_title_encoded}&body={pr_body_encoded}"

                print(f"Pull Request creation URL: {pr_creation_url}")
                print(f"::set-output name=pr_creation_url::{pr_creation_url}") # Output for GitHub Actions

            except subprocess.CalledProcessError as e:
                print(f"Error during Git operations or URL creation: {e}")
                exit(1)
            break
