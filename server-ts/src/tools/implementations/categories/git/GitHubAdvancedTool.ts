...
import {
    CreateBranchFromMainParams,
    CreatePullRequestWithTemplateParams,
    AddCommentToIssueParams,
    GetDiffBetweenBranchesParams,
    CherryPickCommitParams
} from './types/GitHubAdvancedTypes';

const execAsync = promisify(exec);

export class CreateBranchFromMainTool implements ITool {
    public readonly name = 'create_branch_from_main';

    public async execute(params: CreateBranchFromMainParams, context?: ToolContext): Promise<ToolResult> {
        try {
...
export class CreatePullRequestWithTemplateTool implements ITool {
    public readonly name = 'create_pull_request_with_template';

    public async execute(params: CreatePullRequestWithTemplateParams, context?: ToolContext): Promise<ToolResult> {
        try {
            // Validate required parameters
            ...
            if (params.template) {
                try {
                    const templatePath = join(workingDir, '.github', 'pull_request_template.md');
...
export class AddCommentToIssueTool implements ITool {
    public readonly name = 'add_comment_to_issue';

    public async execute(params: AddCommentToIssueParams, context?: ToolContext): Promise<ToolResult> {
        try {
            // Validate required parameters
            ...
            const response = await axios.post(
                `https://api.github.com/repos/${repoFullName}/issues/${params.issue_number}/comments`,
                commentData,
                {
                    headers: {
                        'Authorization': `token ${githubToken}`,
            ...
            if (params.close_issue) {
                await axios.patch(
                    `https://api.github.com/repos/${repoFullName}/issues/${params.issue_number}`,
                    { state: 'closed' },
                    {
                        headers: {
                            'Authorization': `token ${githubToken}`,
...
export class GetDiffBetweenBranchesTool implements ITool {
    public readonly name = 'get_diff_between_branches';

    public async execute(params: GetDiffBetweenBranchesParams, context?: ToolContext): Promise<ToolResult> {
        try {
            // Validate required parameters
            ...
            if (params.summary_only) {
                const { stdout: summaryOutput } = await execAsync(
                    `git diff --name-status ${params.base_branch}...${params.compare_branch}${params.file_filter ? ' -- ' + params.file_filter : ''}`,
                    execOptions
                );
            ...
            const { stdout: statsOutput } = await execAsync(
                `git diff --stat ${params.base_branch}...${params.compare_branch}${params.file_filter ? ' -- ' + params.file_filter : ''}`,
                execOptions
            );
...
export class CherryPickCommitTool implements ITool {
    public readonly name = 'cherry_pick_commit';

    public async execute(params: CherryPickCommitParams, context?: ToolContext): Promise<ToolResult> {
        try {
            // Validate required parameters
            ...
            const { stdout: commitInfo } = await execAsync(
                `git log --format="%H|%s|%an|%ad" -1 ${params.commit_hash}`,
                execOptions
            );
